...
export async function fetchReviewsByProductId(productId: number, token: string) {
  try {
    ...
    console.error('Error fetching reviews by product ID:', (error as Error).message);
...
export async function fetchAmountOfReviewsByDate(date: string, token: string) {
  try {
    ...
    console.error('Error fetching amount of reviews by date:', (error as Error).message);
    throw { error: (error as Error).message };
  }
}
...
//!ADMIN ONLY FUNCTION
export async function modifyReviewStatusById(updatedReviewStatus: UpdatedReviewStatus, token: string) {
  try {
    const response = await request
...
export async function deleteReviewByProductId(productId: number, token: string) {
  try {
    const response = await request
